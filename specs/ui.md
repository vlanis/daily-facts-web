...
From 64rem up (landscape iPad and wider) the empty side gutters carry a
fixed, non-scrolling rushnyk-style band. Below that width there is no
gutter, so nothing is shown — on the iPhone, the primary device, this
never appears at all.

`site/ornament.svg` is used as a **CSS mask, not as an image**: it carries
only the shape, and the colour comes from `background-color: var(--accent)`.
That is what lets a single asset follow both themes; an ordinary
`background-image` would need a light and a dark copy. The rule is wrapped
in `@supports` because without mask support the `background-color` would
paint two solid accent bars down the page.

The SVG is generated by `scripts/gen-ornament.mjs` from a character grid.
Edit the grid and re-run rather than editing the SVG, which is a flat list
of ~37 `<rect>`s.

Inset plus band width must stay within the gutter, which is
`(100vw - 52rem) / 2` — narrowest at the 64rem breakpoint, where 6rem of
gutter holds a 1.25rem inset and a 3.5rem band.
